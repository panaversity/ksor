...
describe("ksor init — acceptance (spec clauses 1-3)", () => {
  ...
  it("runs Next.js only through a wrapper that switches its telemetry off — nothing phones home", () => {
    ...
    // 16.3.3's source, the `next dev` parent (cli/next-dev.js:126) never loads
    // the config and still constructs a Telemetry on exit to record the
    // session, so an assignment in the config would have left that event in
    // place with nothing red. And it is a wrapper, not a `VAR=1 next` prefix:
    // that prefix is POSIX syntax, which cmd.exe refuses under pnpm and npm on
    // Windows — a platform CI walks `ksor init` on — so the prefix would have
    // traded a quiet leak for a loud break. The wrapper is where every
    // manager's root script and the deploy's `pnpm build` end up.
    const dir = workDir();
    expect(runInit(["quiet-sor"], dir).status).toBe(0);
    const site = path.join(dir, "quiet-sor", "system", "site");
    const wrapper = path.join(site, "next-no-telemetry.mjs");
    expect(existsSync(wrapper), "system/site/next-no-telemetry.mjs is emitted").toBe(true);
    expect(
      readFileSync(wrapper, "utf8"),
      "the wrapper hands next an environment with NEXT_TELEMETRY_DISABLED set",
    ).toMatch(/NEXT_TELEMETRY_DISABLED: "1"/);
    const { scripts } = JSON.parse(readFileSync(path.join(site, "package.json"), "utf8")) as {
      scripts?: Record<string, string>;
    };
    ...
    for (const [name, body] of runsNext) {
      expect(
        ...
        `system/site/package.json script "${name}" runs next with telemetry ON — ` +
          `README.md says nothing phones home. Run it through the wrapper: node next-no-telemetry.mjs …`,
