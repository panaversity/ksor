...
import { inForce, isDeniedByLedger, parseLedger } from "../record/ledger";
...
function planStage(recordDir: string, development: boolean): StagePlan {
  ...
  for (const concept of checked.concepts) {
    ...
    if (isDeniedByLedger(denials, concept.id)) continue;
  ...
  // an empty VIEWER is a misconfiguration that would otherwise surface as "the
  // record has no documents" against a record full of them.
  if (admitted.length === 0 && checked.concepts.length > 0) {
    refuse(
      "ksor-audience-empty",
