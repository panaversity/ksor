...
- Frontmatter is the KSoR Profile of OKF (the record spec's §2). Required on
  ...
  compare it against every committed version that was stable under the same
  stamp and refuse `ksor-generated-stale` until you move the stamp past the
  edit and re-approve (a frontmatter-only edit, such as adding a `verified`
  entry, is not a body change; `pnpm check` reads no history, so it passes an
