...
| `pnpm preview` exits `3`                                             | no `system/site/out/` yet, or `PORT` is not a port number, or the port is taken — it says which            | build first; or set a free `PORT` (`dev` uses 3000 too)                                 |
