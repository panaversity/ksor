...
| `packages/{platform,content,gateway-kit,content-gateway}/` | the kernel (decision 11): pool discipline, the content corpus store + retrieval + abstention, serving postures, and the content MCP door (one gateway per record — `content-gateway` today; `identity-gateway`, `praxis-gateway` follow). Ships as ONE npm package: `content-gateway` bundles the other three and carries both bins; `platform`/`content`/`gateway-kit` stay private (decision 12 publish revision) |
...
12. **The kernel's dependency set** (2026-08-19, with decision 11; each
    ...
    **`schema.sql` stays the DDL source of truth** (converted from the
    ...
    with verified why-comments). _Revision 2026-08-19 (publish/bundling
    resolved, owner): the kernel ships as ONE npm package — `@panaversity/
ksor-content-gateway` bundles `platform` + `content` + `gateway-kit`
    (workspace devDeps, tsdown `noExternal`) and exposes BOTH bins
    (`ksor-content` for corpus setup, `ksor-content-gateway` for serve), so an
    adopter installs one thing for the whole served rung. `platform`,
    `content`, and `gateway-kit` stay `private: true` forever — dev/test
    workspace packages, never published; the 4-way split was a conversion
    artifact (1:1 oracle mapping), and the real import graph has one leaf.
    `content`'s `schema.sql` ships as `content-gateway/schema/` (build-copied,
    gitignored, resolved via `import.meta.url`). Verified by a real
    `pnpm pack` → `npm install` in a fresh dir: both bins run, schema
    resolves, deps are self-contained. Re-extract `platform`/`gateway-kit` as
    shared packages only when a SECOND gateway (identity/praxis) is a real
    consumer. The published `ksor` CLI keeps its zero-runtime-deps guarantee;
    `ksor serve` resolves and SPAWNS the installed gateway bin (never imports
    it)._
