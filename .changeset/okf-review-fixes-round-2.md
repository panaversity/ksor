...
**The all-draft build is tested by something that runs Next.** A freshly scaffolded project publishes no page, and the fix for the route that used to throw there was covered only by a staging test that never reaches the route module. The scaffold end-to-end suite now builds the untouched starter before approving anything, and asserts that not one draft reaches a page or `llms.txt`.
