...
"records what the infrastructure did" — with no telemetry code in the tree, while
SLSA/Sigstore two rows above was properly disclosed as future. A responsibilities
table that discloses one unbuilt binding and not the other is the failure mode
this product argues against, on the page that makes the argument. Both are now
disclosed together, with the constraint the row's own wording already implies:
