...
Costs, recorded rather than argued away: the served `tools` array grew from 11,960 to 16,734 characters (~2,990 → ~4,184 always-resident tokens; the three definitions' own JSON sums to 16,730 of that, the array adding the brackets and separators), and each `search` hit — and each `read` reply — carries about 262 characters more. `packages/ksor/docs/tool-surface.md` has the re-measured table and says which of its numbers are exact and which are derived.
