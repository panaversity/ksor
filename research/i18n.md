...
How should a research record written in more than one language be structured and managed within the KSoR system, particularly regarding storage, retrieval, site rendering, and agent interaction, to ensure governance, searchability, and consistency across languages and surfaces?

## Evidence

Research was conducted by examining installed Fumadocs and Next.js 16.14.5 source code and running real builds, not relying on documentation. Two adversarial passes were performed, uncovering errors that were corrected within the research.

**Defects already shipping:**

- `app/api/search/route.ts` passed `language: "english"` to `createFromSource`, leading to zero tokens and unsearchable non-Latin documents (e.g., Urdu or Chinese) because English's splitter regex is Latin-only. This was fixed by PR #121.
- `app/layout.tsx:24` hardcodes `<html lang="en">`, needing a source of truth.
- PostgreSQL lacks Urdu text-search configuration, preventing correct `retrieval.text_search_config` declaration for Urdu.

**Fumadocs capabilities (v16.14.5):**

- `defineI18n` supports `languages`, `defaultLanguage`, `hideLocale`, `parser`, `fallbackLanguage`.
- Content can be named `page.ur.mdx` (dot parser) or `ur/page.mdx` (dir parser).
- Works under `output: "export"` with specific configuration (`hideLocale: "never"`, no middleware).
- Traps: `hideLocale` must be `"never"` for static export (otherwise breaks English links), a dead `proxy.ts` builds clean, `/` is a 404 (requires explicit root page), dynamic routes need `generateStaticParams`.
- Critical behavior: a missing translation silently serves the default language's bytes at the localized URL unless `fallbackLanguage: null` is set, which leads to a genuine 404.
- Naming collision: `<doc>.summary.ur.md` (dot parser) is not matched by `!**/*.summary.md` exclusion, while `<doc>.ur.summary.md` parses locale as `"summary"`. `parser: 'dir'` is the only convention that avoids flat namespace collisions. `tinyglobby` (file discovery) and `picomatch` (dev watcher) can disagree.
- Version 16.14.5 duplicates locale-only pages; fix is in 16.15.0 (which includes source API redesign).

**Framework limitations (kernel):**

- No language representation in the kernel (`packages/content/`, `packages/content-gateway/`).
- `chunks.search_tsv` is a stored generated column with one literal configuration, making one database effectively one stemming configuration. Stock PostgreSQL 17 has Arabic and Hindi but no Urdu text-search config.
- `vector_floor` is a single scalar per instance, calibrated over one pooled distribution, which cannot separate two cosine regimes from a bilingual corpus. Cross-lingual numbers (non-English query → English passage) are unmeasured for KSoR's MRL truncation.
- A per-node takedown (`stable_id` match) does not reach a translation in a separate node; a `subtree` takedown withdraws unrelated siblings.
- `MIN_CONTENT_CHARS` (24 code points) is stricter for dense scripts (Urdu ~1.4x, CJK ~4x) in information terms, leading to Japanese sentences being refused.
- The agent surface (`search`, `read`, `outline`) has no language parameter and cannot grow one locally, leading to cross-language vector results and stemmer-favored keyword results.
- Agent surface exposes no governance caveats (`doc_status`, `superseded_by`).
- `gemini-embedding-2` has shipped (8,192-token input vs 001's 2,048), potentially exceeding `HARD_MAX_CHARS` for CJK, leading to failed chunks.

**Staleness:**

- An English source edit with an untranslated translation means the site serves a statement the record no longer makes.
- Abstention is the wrong mechanism for staleness; a rule bound once and honored by both surfaces (like takedown/audience serving seam) is needed.
- The detector `contentHash()` (sha256 over CRLF-normalised body) exists, but three frontmatter splitters currently disagree, leading to different hash values for the same content.
- Supersession changes no body byte, so a source marked `status: superseded` still reports translations as fresh.
- Prior art (MDN) uses `l10n.sourceCommit` to track the source commit hash for translation synchronization.

**Candidate Designs (all leaked):**

- Rendition (`<doc>.<lang>.md`): Ingresed as a node, but site's glob excludes it; `planStage` filters it for internal docs, reaching public build.
- Language column (`lang`/`translation_of` on `content_nodes`): Invalid SQL in `DENIED_CTE`; governance inheritance is ingest-only; migration silently resolves `TextSearchConfigMismatch`.
- Seventh attachment (`<doc>.translation.<lang>.md`): Glob cannot express BCP-47 regex, leading to permanent mismatches between site and door exclusions.

**Gaps in candidate designs:**

- `KSOR_AUDIENCE` x language interaction is unstated.
- Per-language output trees are not disjoint and cannot merge into one deploy without overwriting fixed-path artifacts.
- The two-shell obligation (decision 9) must be satisfied by any route shape or `llms.txt` change, or the contract is weakened.

## Decision

Nothing is currently implemented or decided regarding multilingual support beyond addressing immediate defects. The document serves as a comprehensive ground for a future owner decision on how to integrate multilingual content.

Key derived principles and immediate actions:

- Absence should be honest rather than papered over (e.g., `fallbackLanguage: null` leading to a 404 for missing translations).
- The site's search index bug (hardcoded `language: "english"`) was fixed (PR #121) to allow non-Latin character search.
- `parser: 'dir'` (`ur/page.mdx`) is the only convention that avoids naming collisions between locales and attachment suffixes.
- Before any convention is ratified, an experiment with `tinyglobby` vs `picomatch` is needed due to potential discrepancies between dev and build environments.
- A single rule that classifies a file should live in one place read by both surfaces, rather than hand-typed copies of globs and regexes.
- A governance state (`reviewed_by IS NULL`) must be a positive state in the table, not a rendering decision, to prevent unreviewed machine translations from appearing with impeccable inherited provenance.
- A verified review is an act performed at the **door** (where a subject exists), not merely a line in a file.
- Staleness detection should be based on a canonical splitter exporting the splitter itself, not only the hash, to resolve discrepancies between kernel and site hash calculations.
- The source's `status: superseded` must be accounted for in staleness detection, as it currently doesn't change the body hash, making translations appear fresh for withdrawn documents.
- The shape for tracking translation synchronization should be `lang` + `translation_of` + `source_hash` columns, a `lib/language.ts` serving seam, and a `LANGUAGE_CASES` decision table under decision 18.

## Rejected

- **Hardcoding `language: "english"` for search**: Rejected due to making non-Latin records unsearchable. (Fixed via PR #121).
- **Ignoring missing translations**: The approach of silently serving default language bytes for missing translations is rejected in favor of an explicit 404 by setting `fallbackLanguage: null`.
- **Dot parser for naming locales and attachment suffixes**: Rejected due to naming collisions and inability to satisfy both locale parsing and exclusion rules simultaneously.
- **Rendition design (`<doc>.<lang>.md`)**: Rejected due to ingestion as a node while site's glob excludes it, and `planStage` filtering for internal documents.
- **Language column design (`lang`/`translation_of` on `content_nodes`)**: Rejected due to invalid SQL in `DENIED_CTE`, ingest-only governance inheritance, and silent resolution of `TextSearchConfigMismatch`.
- **Seventh attachment design (`<doc>.translation.<lang>.md`)**: Rejected due to glob's inability to express BCP-47 regex, leading to mismatched exclusions between site and door.
- **Implementing the keyword arm in JS**: Forbidden by decision 20 to escape the one-database-one-stemming constraint.
- **Making `MIN_CONTENT_CHARS` a simple length check**: Decision 22 removed length as the nav proxy but kept this one as a residual floor, implicitly rejecting a pure length-based approach.
- **Abstention as a staleness mechanism**: Rejected as incorrect; staleness is a content state issue, not a cosine floor measurement.

## Reversal

- **Overall multilingual strategy**: The entire approach to building multilingual support would be reversed if the answer to "who reads this?" is two distinct audiences who never cite each other's documents (e.g., a public English site and an internal Urdu one). In such a case, two separate projects would be the honest and correct outcome.
- **PostgreSQL `regconfig` generated column**: If the measurement of whether `GENERATED ALWAYS AS (to_tsvector(<sibling regconfig column>, content)) STORED` is accepted at all fails, the per-row design would collapse, requiring a reversal of this approach.
- **Shared vector space assumption**: If measurements show that same-language and cross-language cosines do not occupy one regime at 1536 dimensions, the entire premise of a shared vector space for multilingual retrieval would be reversed.
- **Fumadocs version**: If bumping Fumadocs to `≥ 16.15.0` (which is a source-API redesign) with a two-shell conformance run reveals critical regressions or insurmountable issues, the framework choice for the site shell could be reconsidered.
- **Decision 9's reversal clause**: The fact that decision 9's reversal clause was narrowed at ratification, removing the condition "or if multilingual i18n becomes a near-term requirement it cannot meet," means that multilingual demand no longer directly re-opens the shell choice. A future explicit owner decision would be required to re-introduce this condition or reverse decision 9 due to i18n needs.
- **Decision 21 regarding verified reviews**: The current `reviewed_by:` frontmatter being a self-asserted string is a violation of decision 21's reversal clause. This implies a future reversal to verify identity at the door, where a subject exists, would be necessary.
- **Staleness detection discrepancies**: If a canonical splitter cannot be established to ensure both kernel and site calculate the same `contentHash()`, leading to permanent disagreement on staleness, the hash-based staleness rule would need reversal.
