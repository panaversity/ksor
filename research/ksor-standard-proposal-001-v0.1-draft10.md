...
**Authoritative Record.** The canonical, governed OKF bundle of Markdown concept documents from which all projections derive, together with the control files that sit beside the bundle at the record root (4.1.2): the instance document, the Governance Policy, and the takedown ledger.

**Concept.** One Markdown file in the bundle, other than a reserved file (4.1.2) or a companion (4.2.1.4), whose bundle-relative path is its identity, as defined by OKF.

**Governance Boundary.** The point at which policy determines which concepts are approved, visible, and current for a given audience. No knowledge crosses a serving or publication boundary without first passing the applicable governance decision. For static publication the decision is evaluated once per audience-specific publication. For dynamic retrieval it is evaluated per request, using the effective audience of that request.
...
**Publication.** One complete, identified publication of the record, recorded in a build lock file under a publication identifier (`build_id` in the reference build lock) that connects all projections produced from it. Drafts 1 to 9 called this a generation. That word is left to implementations: a retrieval layer commonly keeps a monotonic generation counter that a citation pins, and that counter is not the publication identifier.

**Instance Document.** The profile-shaped document `instance.md` at the record root, beside the bundle rather than inside it (4.1.2), describing the identity and purpose of the KSoR instance. It is not an OKF concept and carries no `status` or `ksor.audience`. Its `title` is the heading of the generated bundle-root `index.md`, and its `description` seeds the discovery index (6.2.2).
...
**Takedown.** A governed decision, issued under the Governance Policy's takedown authority and recorded per R27 as an entry in the takedown ledger (4.2.5.4), that overrides publication status and triggers the removal obligations of R9.
...
**Class B: Conformant Publisher.** A build system that projects a Class A corpus into surfaces while satisfying the governance requirements applicable to publication and authoring (Section 9, R1 to R6, R9 to R14, R21, and R22 to R27), including validation of the corpus against the Governance Policy (4.2.5.3) and of the takedown ledger (4.2.5.4).
...
For a static build, that decision runs once per audience-specific publication. For dynamic retrieval, it runs per request. In both cases the policy semantics are identical, and no downstream surface decides independently whether something should have been published.
...
**4.1.2** Because the record is literally an OKF bundle, OKF's filesystem contract applies in full. The bundle root is the `knowledge/` directory of the record. The instance document `instance.md`, the Governance Policy `.ksor/governance.yaml`, the takedown ledger `.ksor/takedowns.yaml`, and the build lock `build.lock.json` are siblings of the bundle at the record root, not members of it, so a bare OKF consumer handed `knowledge/` sees a conformant bundle and nothing of the instance, the site, or the system. `index.md` and `log.md` are reserved at every directory level and MUST NOT be used as concept documents [okf-spec], and this profile additionally reserves `README.md` (4.2.1.3). An `index.md` follows OKF's progressive-disclosure listing format and carries no frontmatter, except that the bundle-root `index.md` MAY carry `okf_version`. A `log.md` follows OKF's date-grouped history format. A consequence for the human surface is that directory landing pages MUST be rendered from, or generated alongside, the OKF `index.md` files rather than authored as ordinary content pages under those names. Publishers SHOULD generate `index.md` files mechanically at build time, which satisfies both the OKF contract and the discovery surface with one artefact. The bundle-root `index.md` takes its heading from the instance document's `title`, and only its heading: an OKF index is headings and bullets, so it carries no summary, and the instance `description` seeds the discovery index instead (6.2.2). Instance identity is therefore authored once, in `instance.md`, and projected into both the OKF listing and the discovery index. The committed `index.md` files in the bundle are the record's own map and list every concept regardless of `status` or `ksor.audience`, since anyone holding the repository holds the files. Every `index.md` that a projection carries, whether in a per-audience site, an exchange bundle, or a discovery artefact, is a filtered projection regenerated from the set admitted for that audience, never a copy of the committed map (R5, R12), and R13 applies to those projections, not to the committed map. In a generated index a subdirectory bullet carries no description, a deviation from OKF §8's SHOULD that is recorded here: a directory is a container, and a concept describes itself.
...
The profile constrains OKF for governed use. It is stricter than OKF and permitted by it, since OKF preserves custom conventions rather than rejecting them. OKF §11 directs consumers never to reject a bundle for a missing optional field, an unknown type, or an unknown key [okf-spec]. The build failures this profile requires do not contradict that, because profile validation of one's own record by its publisher is not OKF consumption: a Class B publisher enforcing the profile against the record it publishes acts as the record's producer, while a Class E importer reading another party's bundle remains a tolerant OKF consumer (6.4.2).
...
**4.2.1.2** `Document` is the profile's named non-reserved default type. A concept that declares `type: Document` is subject to no type-keyed rule, so it needs neither `sources` nor `ksor.owner`, and this profile will never reserve the name, so that a record can begin with no governance beyond the required fields of 4.2.2 and climb from there.

**4.2.1.3** `README.md` is reserved by this profile at every directory level, in addition to the `index.md` and `log.md` that OKF reserves, and MUST NOT be used as a concept document. OKF itself reserves only `index.md` and `log.md` and would read a `README.md` as a concept, which is why the reservation is stated here rather than inherited: the name conventionally describes a directory, and `index.md` is where that belongs.

**4.2.1.4** A companion file `<doc>.summary.md` beside a concept `<doc>.md` carries the frontmatter `type: Summary` and nothing else. `Summary` is a companion marker, not a concept type, and MUST NOT be used as the `type` of a concept. A companion is governed entirely by its parent: it has no identity, no `ksor.audience`, no `status`, no approval, and no entry of its own in any projection, and it inherits its parent's audience, lifecycle, and takedown. Under bare OKF such a file is an ordinary concept with an identity of its own, so the no-independent-identity guarantee is a rule of this profile, and a Class B publisher MUST enforce it: a companion carrying any other frontmatter key, or one with no parent concept, MUST fail the build.
...
**4.2.2.3** Every concept with `status: stable` MUST carry `generated`, including `generated.at`, and `ksor.approval`. OKF leaves `generated.at` optional [okf-spec]; this profile REQUIRES it on stable concepts so that the comparison R23 makes has no undefined branch. `generated.at` MUST identify the most recent substantive content change. `verified` is not required for `status: stable`. Drafts 6 to 9 required at least one `verified` entry, and that coupled verification to publication: an author with an approver but no reviewer would have had to write one, manufacturing exactly the event R17 forbids deriving from approval. A stable, approved, unverified concept is an honest state, and it sits at trust tier unverified (4.2.3.1), which is what that tier exists to name.
...
**4.2.2.5** A Class B publisher MUST fail the build when these requirements, or the governance extension requirements of 4.2.4, are violated. This is profile validation of the record by its own publisher, not OKF consumption (4.2), so it does not contravene OKF's rule that a consumer MUST NOT reject a concept for a missing optional family [okf-spec]; a Class E importer reading another party's bundle remains tolerant (6.4.2).

**4.2.2.6** The profile defines one top-level key of its own, `order`: an OPTIONAL integer that orders a concept among its siblings in generated `index.md` files and navigation (4.1.2). It carries no governance meaning, and OKF consumers preserve it as an unknown key.

#### 4.2.3 Trust Vocabulary

The profile adopts the OKF v0.2 trust vocabulary without modification [okf-spec] [okf-v02]:

| Governance concern                | OKF field                                                                                                                                                                                         |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Lifecycle                         | `status: draft \| stable \| deprecated`                                                                                                                                                           |
| Provenance and per-claim citation | `sources`, each entry with `resource`; a claim cites a source by a footnote whose reference and definition are both keyed to a `sources[].id` (4.2.3.5)                                           |
| Review evidence                   | `verified: [{ by, at }]`; a bare `{ by, at }` mapping is a one-element list (4.2.3.3)                                                                                                             |
| Review period                     | `stale_after` (an ISO 8601 instant, stale when `now >= stale_after`)                                                                                                                              |
| Production history                | `generated: { by, at }`                                                                                                                                                                           |
| Sanctioned computation            | `type: Attested Computation`, with `runtime`, the only field OKF requires for the type; `parameters`, `computation`, `executor`, and `attester` are described by OKF §10.2 and not required by it |
...
**4.2.3.3** `verified` is a list of `{ by, at }` entries. OKF permits a single verifier to be written as a bare `verified: { by, at }` mapping and requires every consumer to treat it as a one-element list [okf-spec]. A conformant implementation MUST accept the bare mapping as a one-element list, and a Class B publisher MUST NOT fail the build on the bare form.

**4.2.3.4** Every timestamp in the profile, `generated.at`, `verified[].at`, `stale_after`, `ksor.approval.at`, `ksor.deprecated.at`, and `ksor.effective_from`, is an ISO 8601 instant with an explicit UTC offset, as OKF requires of every timestamp-valued key [okf-spec]. `stale_after` is an instant, and a concept is stale when `now >= stale_after`. A bare `YYYY-MM-DD` date is not an instant: a publisher SHOULD refuse one rather than guess an offset, and migration tooling MAY widen one to midnight UTC.

**4.2.3.5** Per-claim citation is a GFM footnote whose label is a `sources[].id`. Both the footnote reference in the body and the footnote definition MUST be keyed to a source id the concept declares, and a Class B publisher MUST fail a reference or a definition whose label matches no `sources[].id`. Footnotes are the profile's one extension to CommonMark, and they degrade to readable text in a renderer without them.

_Compatibility note (non-normative)._ KSP-001 version 0.1 follows the OKF v0.2 specification at the commit identified by [okf-spec]. Upstream reference code and later edits on moving branches are informative until a KSP revision adopts them. In that pinned revision every timestamp-valued key is an ISO 8601 instant with an explicit offset, `stale_after` is an instant, and staleness is evaluated as `now >= stale_after`. Drafts 6 to 9 pinned an earlier snapshot in which `stale_after` was a `YYYY-MM-DD` date evaluated as `today >= stale_after`; upstream replaced dates with instants on 2026-08-21 under the unchanged `0.2` label, which is why the pin is a commit and not a version (13.3).
...
**4.2.4.2** `ksor.audience` is a required list of audience identifiers naming who may see the concept. `public` is a reserved identifier denoting the unrestricted audience, and a policy document MUST NOT redefine it. Public knowledge MUST state `ksor.audience: [public]` explicitly. Absence of `ksor.audience` is invalid and MUST fail a Class B build. Implementations MUST NOT infer public visibility from omission. A viewer, whether a request or a static build, holds a list of audience identifiers that MUST include `public`, and a concept is visible to that viewer when the two lists overlap: membership stays on the concept, and no identifier outranks another. A link, a `ksor.superseded_by` pointer (4.2.4.6), or the body of a companion (4.2.1.4, evaluated with its parent's audience) is permitted only when the target's audience list contains `public` or contains every identifier in the source's list, so that every reader of the source may read the target (R11). This field is the canonical audience representation. Retrieval predicates (Section 5), publication filters (R5, R6), and exchange bundles (R15) MUST all derive from it. All other audience identifiers are defined in the Governance Policy's audience registry (4.2.5), and mapping identity claims onto them is local policy (8.1.2).
...
**4.2.4.5** `ksor.deprecated` records the authority decision to deprecate: `{ by, at }` in the actor convention. Every concept with `status: deprecated` MUST carry `ksor.deprecated`, granted by the concept's owner as resolved by the Governance Policy (4.2.5) or by an actor its `takedown_authorities` names. This is the deprecation clause of R23 made mechanically checkable, in the same way that `ksor.approval` makes the transition to `stable` checkable.

**4.2.4.6** `ksor.superseded_by` is an optional bundle-relative concept path naming the concept that replaces a deprecated one, so that a human surface can show a deprecated concept together with its successor. A Class B publisher MUST fail the build when the target does not exist, is not `status: stable`, or fails the audience rule of 4.2.4.2, since a supersession pointer discloses its target exactly as a link does (R11).
...
The four named governance families and the keys shown above are normative: `approval_authorities` and `takedown_authorities` are required, and `audiences` and `ownership` are optional (4.2.5.1). Implementations MAY preserve additional extension keys, but MUST NOT assign conflicting meaning to the standard keys.
...
**4.2.5.1** A conformant corpus MUST include `.ksor/governance.yaml`. The **audiences** map is OPTIONAL. When present it names every audience identifier used in `ksor.audience` other than the reserved `public`, and each entry MUST have a human-readable `description`; when absent, no identifier is registered and every concept's `ksor.audience` MUST be `[public]`. The **ownership** list is OPTIONAL. When present it resolves which owner governs each applicable scope, each rule MUST provide `owner`, and `escalation` names the authority for ownership changes under R24; when absent, no owner resolves and R24 binds nothing, an absence the publisher discloses rather than fills with a default. The **approval_authorities** list is REQUIRED and names which actors may grant `ksor.approval` for each scope; each rule MUST provide a non-empty `actors` list. The **takedown_authorities** mapping is REQUIRED and MUST provide a non-empty `actors` list naming actors who may issue, revoke, and amend takedowns under R27 and 4.2.5.4. A policy holding only the two required families is the smallest conformant policy.
...
**4.2.5.3** A Class B publisher MUST validate the corpus against the Governance Policy and fail the build on violations: missing or unreadable `.ksor/governance.yaml`, audience identifiers absent from the registry, approvals granted by unauthorised actors, deprecations recorded by an actor who is neither the resolved owner nor a takedown authority (4.2.4.5), takedown ledger entries by actors the takedown authority does not name (4.2.5.4), and changes that bypassed the ownership map.

**4.2.5.4** The takedown record is an append-only ledger at `.ksor/takedowns.yaml`, beside the Governance Policy and, like it, part of the authoritative record without being an OKF concept. Every entry, whether a denial, a revocation of an earlier denial, or an amendment of one, names its actor and its instant, and that actor MUST be one the `takedown_authorities` list names. Lifting a takedown is a new entry naming the entry it revokes, never a removed line. A Class B publisher MUST validate every entry against the policy whoever wrote it, since a committed file is something anyone with write access can append to, and MUST fail the build when the ledger has lost an entry it previously contained.
...
`status = 'stable'` is the concept's lifecycle (4.2.3), the two instant comparisons are staleness and effectivity evaluated at request time (4.2.3.4, 4.2.4.4), and `trust_tier` holds the reference encoding of 4.2.3.1: a small integer ordered unverified < machine-confirmed < human-reviewed, so that `>=` against the requester's floor is exactly the ordering R18 requires and nothing more. A takedown denial (4.2.5.4) is a further conjunct of the same predicate. `audience` is never NULL for a valid concept because `ksor.audience` is explicit and required on every concept (4.2.2.4 and 4.2.4.2). `&&` is array overlap: the concept is visible when any of its audiences matches any audience the requester holds. A request to an open surface includes the reserved `public` audience. Containment is the wrong test here, because a requester holding several audiences must not be denied a concept that any one of them entitles them to.
...
**6.1.2** The human surface SHOULD render the trust signals present in frontmatter: status, verifying identity and date, and staleness instant.
...
**6.2.5** Every discovery artefact MUST carry the publication identifier, corpus commit hash, and implementation version (R14).
...
**Rung 2: Governed interaction (MCP).** Guarantees: retrieval filters on governance and trust before ranking (Section 5), results carry trust signals (6.3.2), answers are citable (6.3.3), abstention is enforced (6.3.4), and responses are traceable to a publication (R21). Narrower reach, real guarantees.
...
**8.2.1** Builds MUST record a publication lock, the build lock file of 2.2 (included documents, hashes, source commit, versions, and the publication identifier).
...
**R1. No bypass of the record.** All published and served knowledge MUST originate from the authoritative record or a publication derived from it. Every page, row, entry, result, and bundle MUST be traceable back to the record.
...
**R11. Links can disclose.** Cross-audience references MUST be validated: a link, a supersession pointer, or a companion body may reach a target only when the target's `ksor.audience` contains `public` or contains every identifier in the source's `ksor.audience` (4.2.4.2). A public link to a restricted concept discloses its existence, name, subject, and route, and is a governance violation.
...
**R14. Artefacts carry their publication.** Every generated machine artefact MUST carry the publication identifier, corpus commit hash, and implementation version.
...
**R21. One identifier per publication.** All projections of a publication MUST carry the same publication identifier, and a consumer SHOULD be able to establish which publication it is using. The identifier MUST differ whenever the admitted set differs, including when the instant at which a build evaluates effectivity and staleness crosses a boundary, so that two publications with different content never share one identifier.
...
**R23. Status transitions are governed events.** A transition to `stable` MUST carry `generated` with `generated.at` (4.2.2.3) and a valid `ksor.approval` granted by an actor the Governance Policy authorises for that concept's path or type. A reviewed change that alters a stable concept MUST update `generated.at` and either renew `ksor.approval` within the same change or set `status: draft`. The check is mechanical: a publisher MUST fail a stable concept when `generated` or `generated.at` is absent or when `generated.at` postdates `ksor.approval.at`. A transition to `deprecated` MUST be made under owner or takedown authority and MUST be recorded in `ksor.deprecated` (4.2.4.5), which the publisher checks the same way.

**R24. Ownership binds review.** A change to a concept of a reserved type MUST be reviewed under the authority of its `ksor.owner`, as resolved by the Governance Policy's ownership map. A change to `ksor.owner` itself MUST be authorised by the outgoing owner or by the escalation authority named in the Governance Policy. Where the Governance Policy declares no ownership map (4.2.5.1), no owner resolves and this rule binds nothing; the absence is disclosed, never filled with a default. Ownership that binds nothing is decoration. This rule is what makes it governance.

**R25. Recorded actors are truthful.** `generated.by`, `verified[].by`, `ksor.approval.by`, `ksor.deprecated.by`, and the actor of every takedown ledger entry MUST record the actor that actually performed the action. An agent MUST NOT record a `human:` actor for an action it performed, and MUST NOT grant `ksor.approval` on its own authority. Where the change-control platform exposes the committing or reviewing identity, a publisher MUST fail the build when that identity contradicts the asserted actor.
...
**R27. Takedown is a governed write.** A takedown decision MUST be issued by an actor the Governance Policy's takedown authority names and MUST be recorded in the change-controlled record as an entry in the append-only takedown ledger (4.2.5.4), as MUST every revocation or amendment of it, so that the removal obligation of R9 has an auditable trigger and an auditable author, and so that lifting a takedown is itself an auditable act.
